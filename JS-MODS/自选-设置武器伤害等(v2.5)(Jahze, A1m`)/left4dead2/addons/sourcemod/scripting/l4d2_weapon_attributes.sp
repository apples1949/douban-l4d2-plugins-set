...
{
	hHideWeaponAttributes = CreateConVar("sm_weapon_hide_attributes","2", "允许玩家使用 'sm_weapon_attributes' 命令查询武器属性. 0=禁用命令, 1=只限管理员. 2=所有玩家.", _, true, 0.0, true, 2.0);
...
{
	if (args < 3) {
		PrintDebug(eLogError|eServerPrint, "用法: sm_weapon <weapon> <attr> <value>.");
	...
	if (!L4D2_IsValidWeapon(sWeaponName)) {
		PrintDebug(eLogError|eServerPrint, "武器名称错误:%s.", sWeaponName);
		return Plugin_Handled;
	}
	...
	if (iAttrIdx == -1) {
		PrintDebug(eLogError|eServerPrint, "属性名称错误:%s.", sAttrName);
		return Plugin_Handled;
	}
...
public Action Cmd_WeaponAttributes(int client, int args)
{
	...
	if (args < 1) {
		ReplyToCommand(client, "用法: sm_weapon_attributes <weapon>.");
		return Plugin_Handled;
	}
	...
	if (!L4D2_IsValidWeapon(sWeaponName)) {
		ReplyToCommand(client, "武器名称错误: %s.", sWeaponName);
		return Plugin_Handled;
	}

	ReplyToCommand(client, "武器 %s 的详细数据:", sWeaponName);
...
public Action Cmd_WeaponAttributesReset(int args)
{
	...
	if (iCount == 0) {
		PrintToServer("武器属性没有重置，因为没有保存武器属性!");
		return Plugin_Handled;
	}
	
	PrintToServer("已成功重置所有保存武器的武器属性。重置武器属性的数量:%d!", iCount);
