...
void GetAllModeMenu(int client, int index)
{
	char line[128];
...
int Menu_HandlerAllMode(Menu menu, MenuAction action, int client, int itemNum)
{
	switch(action)
	{
		...
		case MenuAction_Select:
		{
			...
			{
				case 0:
					IsRestartGameMenu(client, sName);
				case 1:
					GetGameModeMenu(client, 0, sName, g_sModeCoop, sizeof(g_sModeCoop));
				case 2:
					GetGameModeMenu(client, 0, sName, g_sModeVersus, sizeof(g_sModeVersus));
				case 3:
					GetGameModeMenu(client, 0, sName, g_sModeSingle, sizeof(g_sModeSingle));
			}
		}
...
void IsRestartGameMenu(int client, char[] g_sModeName)
{
	if(g_hGameMode == null)
	{
		char line[32];
		Menu menu = new Menu(Menu_HandlerRestartGameMenu);
		Format(line, sizeof(line), "%s:", g_sModeName);
		SetMenuTitle(menu, "%s", 	line);
		menu.AddItem("0", "确认");
		menu.AddItem("1", "取消");
		menu.ExitButton = true;//默认值:true,设置为:false,则不显示退出选项.
...
void GetGameModeMenu(int client, int index, char[] g_sModeName, char[][][] g_sModeCode, int g_iModeCode)
{
	char line[128], sInfo[128], sData[2][64];
