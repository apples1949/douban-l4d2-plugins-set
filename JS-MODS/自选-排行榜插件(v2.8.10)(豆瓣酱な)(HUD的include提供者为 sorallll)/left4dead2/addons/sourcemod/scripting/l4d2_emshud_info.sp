...
float g_fMapRunTime, g_fMeleeNerf2, g_fTempValue;
...
void IsPlayersNumber()
{
	...
	if(g_iPlayersNumber & Number_64)
	{
		...
		if(g_iNumber[6] > 0)
			FormatEx(g_sInfo[6], sizeof(g_sInfo[]), "生还:%d ", g_iNumber[6]);
	}
...
void IsDeathMessage()
{
	...
	HUDPlace(HUD_FAR_LEFT,0.00,g_iShowServerName <= -1?0.13:0.155,1.0,0.22);
}
...
void IsShowRunningTime()
{
	...
	HUDPlace(HUD_SCORE_TITLE,0.00,g_iShowServerName <= -1?0.110:0.135,1.0,0.03);
}
...
int GetMaxPlayers()
{
	int g_iMaxPlayers = GetConVarInt(FindConVar("sv_maxplayers"));
	if(g_iMaxPlayers == -1)
	{
