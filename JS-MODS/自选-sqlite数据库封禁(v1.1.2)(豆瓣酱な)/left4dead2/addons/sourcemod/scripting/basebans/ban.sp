...
void PrepareBan(int client, int target, int time, const char[] reason)
{
	...
	if (reason[0] == '\0')
	{
		BanClient(target, time, BANFLAG_AUTO|BANFLAG_NOKICK, "Banned", "Banned", "sm_ban", client);
	}
	else
	{
		BanClient(target, time, BANFLAG_AUTO|BANFLAG_NOKICK, reason, reason, "sm_ban", client);
	}
}
