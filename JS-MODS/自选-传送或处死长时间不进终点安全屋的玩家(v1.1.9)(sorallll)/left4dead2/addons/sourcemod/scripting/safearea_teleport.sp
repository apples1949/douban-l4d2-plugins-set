...
public void OnPluginStart()
{
	...
	g_hCvarModesTog =		CreateConVar("st_modes_tog",	"0",	"在那些游戏模式中打开插件(将数字相加). 0=全部, 1=战役, 2=生存, 4=对抗,8=清道夫.", CVAR_FLAGS);

	g_hSafeAreaFlags =		CreateConVar("st_enable",		"3",	"在哪里启用传送? (1=终点安全屋, 2=终点救援车, 3=两者)", CVAR_FLAGS);
	g_hSafeAreaType =		CreateConVar("st_type",			"1",	"倒计时结束后没有进入安全区的幸存者如何处理. (1=传送, 2=处死).", CVAR_FLAGS);
	g_hSafeAreaTime =		CreateConVar("st_time",			"15",	"设置倒计时的时间/秒.", CVAR_FLAGS);
	g_hMinSurvivorPercent =	CreateConVar("st_min_percent",	"50",	"有多少幸存者进入终点安全区后开始倒计时(百分比).", CVAR_FLAGS);
