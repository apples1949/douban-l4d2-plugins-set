	...
	switch (action) {
		case MenuAction_Select: 
		{
			if (param2 >= 0 && param2 <= 7)
			{
				char item[32];
				menu.GetItem(param2, item, sizeof item);
				SetCharacter(client, GetClientOfUserId(StringToInt(item)), true, param2, param2);
			}
		}
		case MenuAction_End:
			delete menu;
...
int Csm_MenuHandler(Menu menu, MenuAction action, int client, int param2) {
	switch (action) {
		case MenuAction_Select: {
			if (param2 >= 0 && param2 <= 7)
				SetCharacter(client, client, false, param2, param2);
		}
...
bool CanUse(int client, bool checkAdmin = true) {
	...
	{
		ReplyToCommand(client, "旁观者无法使用该指令.");
		return false;
	}
...
Action cmdZoeyUse(int client, int args) {
	...
	SetCharacter(client, client, false, ZOEY);
	return Plugin_Handled;
}
...
Action cmdNickUse(int client, int args) {
	...
	SetCharacter(client, client, false, NICK);
	return Plugin_Handled;
}
...
Action cmdEllisUse(int client, int args) {
	...
	SetCharacter(client, client, false, ELLIS);
	return Plugin_Handled;
}
...
Action cmdCoachUse(int client, int args) {
	...
	SetCharacter(client, client, false, COACH);
	return Plugin_Handled;
}
...
Action cmdRochelleUse(int client, int args) {
	...
	SetCharacter(client, client, false, ROCHELLE);
	return Plugin_Handled;
}
...
Action cmdBillUse(int client, int args) {
	...
	SetCharacter(client, client, false, BILL);
	return Plugin_Handled;
}
...
Action cmdBikerUse(int client, int args) {
	...
	SetCharacter(client, client, false, FRANCIS);
	return Plugin_Handled;
}
...
Action cmdLouisUse(int client, int args) {
	...
	SetCharacter(client, client, false, LOUIS);
	return Plugin_Handled;
}
...
void SetCharacter(int client, int victim, bool type, int character, int modelIndex) {
