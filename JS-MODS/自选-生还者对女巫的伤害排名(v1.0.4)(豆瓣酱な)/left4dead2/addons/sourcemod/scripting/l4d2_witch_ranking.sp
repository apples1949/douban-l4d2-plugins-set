...
public void OnPluginStart()
{
	...
	HookEvent("player_death", Event_PlayerDeath);//女巫死亡.
...
public void Event_WitchSpawn(Event event, const char[] name, bool dontBroadcast)
{
	...
	if (IsValidEdict(Witchid))
	{
		...
		IsResetVariable(Witchid);//女巫出现时重置整型变量.
	}
}
...
void GetFrameHealth(DataPack hPack)
{
	...
	if (IsValidEdict(Witchid))
		g_fWitchHP[Witchid] = GetWitchHealth(Witchid);//记录女巫出现时的血量.
	delete hPack;
}

float GetWitchHealth(int Witchid)
{
	return float(GetEntProp(Witchid, Prop_Data, "m_iHealth"));
}
...
public Action OnTakeDamageAlive(int client, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3])
{
	if(IsValidClient(attacker) && GetClientTeam(attacker) == 2)
	{
		if (IsValidEdict(client))
		{
			...
			//float fHealth = GetWitchHealth(client);
			g_fWitchHurt[client] = GetWitchHealth(client);//记录坦克剩余的血量.
			g_fSurvivorWitchHurt[!iBot ? attacker : iBot][client] += damage > g_fWitchHurt[client] ? 0.0 : damage;
			//PrintToChatAll("受伤,血量:%f伤害\x05:%f", GetWitchHealth(client), damage);
		}
	}
...
{
	char sName[32];
	int Witchid = event.GetInt("entityid");
	GetEdictClassname(Witchid, sName, sizeof(sName));
	if (IsValidEdict(Witchid) && strcmp(sName, "witch") == 0)
	{
		...
		if(IsValidClient(attacker) && GetClientTeam(attacker) == 2)
		{
			...
			{
				int iBot = IsClientIdle(attacker);
				g_fSurvivorWitchHurt[!iBot ? attacker : iBot][Witchid] += g_fWitchHurt[Witchid];//坦克死亡后把剩余血量+给击杀者.
			}
		}
		if (g_iWitchRanking > 0)
			IsWitchDamageSort(Witchid, GetWitchIndex(Witchid), "死亡");
		IsResetVariable(Witchid);//女巫被死亡时重置整型变量.
	}
}		
//获取自定义的女巫编号.
