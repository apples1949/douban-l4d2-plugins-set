	...
	if(IsCheckClientAccess(client))
	{
		switch(args)
		{
			...
			default:
			{
				...
				PrintToChat(client, "\x04[提示]\x05已设置新服名为\x04:\x05(\x03%s\x05)\x04(%s).", buffer, GetHostPort());
			}
		}
	}
	else
...
void ReadFileContent(char[] sHostName, char[] sHostPort)
{
	if (!FileExists(g_sKvPath))//没有配置文件.
	{
		...
		//写入默认内容.
		kv.SetString("default", sHostName);
		// 返回上一页.
		kv.Rewind();
		// 把内容写入文件.
		kv.ExportToFile(g_sKvPath);//写入文件成功.
	...
	else
	{
		...
		if (kv.ImportFromFile(g_sKvPath))//文件读取成功.
		{
			char sData[2][128];
			// 获取Kv文本内信息写入变量中.
			kv.GetString("default", sData[0], sizeof(sData[]), sHostName);
			kv.GetString(sHostPort, sData[1], sizeof(sData[]), sData[0]);
			kv.Rewind();//返回上一层.
			kv.ExportToFile(g_sKvPath);//把数据写入到文件.
			g_hHostName.SetString(sData[1]);//设置服务器名称.
		}
		else//文件读取失败.
		{}
...
void WriteFileContent(char[] sHostName, char[] sHostPort)
{
	if (!FileExists(g_sKvPath))//没有配置文件.
	{
		...
		//写入默认内容.
		kv.SetString("default", DEFAULT_NAME);
		kv.SetString(sHostPort, sHostName);
		// 返回上一页.
		kv.Rewind();
		// 把内容写入文件.
		kv.ExportToFile(g_sKvPath);//写入文件成功.
