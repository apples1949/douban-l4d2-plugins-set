...
float  g_fKeyboardTimer, g_fKeyboardCooling, g_fSpecnextTimer, g_fSpecnextCooling;
ConVar g_hKeyboardTimer, g_hKeyboardCooling, g_hSpecnextTimer, g_hSpecnextCooling;
...
public void OnPluginStart()
{
	...
	g_hKeyboardTimer = CreateConVar("l4d2_away_timer_keyboard", "1.5", "设置多少秒后自动加入观察者. 0=禁用.", FCVAR_NOTIFY);
	g_hKeyboardCooling = CreateConVar("l4d2_away_keyboard_cooling", "5.0", "设置自动加入观察者冷却时间.", FCVAR_NOTIFY);
	g_hSpecnextTimer = CreateConVar("l4d2_away_timer_specnext", "2.0", "设置多少秒后自动加入幸存者. 0=禁用.", FCVAR_NOTIFY);
	g_hSpecnextCooling = CreateConVar("l4d2_away_specnext_cooling", "5.0", "设置自动加入幸存者冷却时间.", FCVAR_NOTIFY);
	g_hKeyboardTimer.AddChangeHook(ConVarValueChanged);
	g_hKeyboardCooling.AddChangeHook(ConVarValueChanged);
	g_hSpecnextTimer.AddChangeHook(ConVarValueChanged);
	g_hSpecnextCooling.AddChangeHook(ConVarValueChanged);
...
void GetConVarValue()
{
	...
	g_fKeyboardCooling = g_hKeyboardCooling.FloatValue;
	g_fSpecnextTimer = g_hSpecnextTimer.FloatValue;
	g_fSpecnextCooling = g_hSpecnextCooling.FloatValue;
}
//生还者使用闲置命令.
		...
		switch (GetClientTeam(client))
		{
			...
			case 2:
			{
				...
				else
				{
					...
					{
						PrintToChat(client, "\x04[提示]\x05至少两名玩家才能使用休息.");
						return Plugin_Handled;
					}

					float fFromTime = GetEngineTime() - g_fAwayTime[client];
					if (g_fKeyboardTimer > 0.0 && fFromTime < g_fKeyboardCooling)
					{
						PrintCenterText(client, "请等待 %.1f 秒后再使用休息.", g_fKeyboardCooling - fFromTime);
						return Plugin_Handled;
					}
					...
					else 
					{
						if(g_hAwayTimer[client] == null)
						{
							...
							PrintCenterText(client, "你将在 %.1f 秒后自动闲置.", g_fKeyboardTimer);
...
Action AddSpecNext(int client, char[] command, int argc)
{
	...
	if(IsValidClient(client) && !IsFakeClient(client))
	{
		if(GetClientTeam(client) == 1 && iGetBotOfIdlePlayer(client))
		{
			...
			if (fFromTime < g_fSpecnextCooling)
			{
				PrintCenterText(client, "请等待 %.1f 秒后再加入幸存者.", g_fSpecnextCooling - fFromTime);
				return Plugin_Handled;
			}
			...
			if(g_hTakeTimer[client] == null)
			{
				...
				PrintCenterText(client, "你将在 %.1f 秒后自动加入.", g_fSpecnextTimer);
