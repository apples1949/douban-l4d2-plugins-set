...
#define PLUGIN_VERSION	"1.3.8"	//定义插件版本.

char g_sWrite[][][] = 
{
	{"指令设置人数时写入文件. 0=禁用, 1=启用.", "1"},
	{"设置服务器的最大最大人数(1~31). -1=默认.", "8"}
};
...
void vCreateReadFile()
{
	...
	else if (kv.ImportFromFile(g_sKvPath)) //文件存在就导入kv数据,false=文件存在但是读取失败.
	{
		...
		{
			char sData[128];
			kv.GetString(g_sWrite[1][0], sData, sizeof(sData), g_sWrite[1][1]);//获取文件里指定的内容.

			int iPlayers = StringToInt(sData);

			if(iPlayers != 0)
				SetMaxPlayers(GetMaxNumber(g_bMaxPlayers == false ? iPlayers : g_iMaxPlayers));//设置服务器最大人数.
		}
...
void VUpdateConfigurationFile(char[] sMaxPlayers)
{
	...
	{
		char sData[128];
		kv.GetString(g_sWrite[0][0], sData, sizeof(sData), g_sWrite[0][1]);//获取文件里指定的内容.

		if(StringToInt(sData) == 0)
		{
			g_bMaxPlayers = StringToInt(sMaxPlayers) == StringToInt(sData) ? false : true;
		}
		else
		{
			g_bMaxPlayers = false;
