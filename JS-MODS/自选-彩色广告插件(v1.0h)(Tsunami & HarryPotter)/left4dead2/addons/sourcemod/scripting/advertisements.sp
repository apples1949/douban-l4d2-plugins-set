...
public void OnPluginStart()
{
    ...
    g_hEnabled  = CreateConVar("sm_advertisements_enabled",  "1",                  "开启广告插件. 0=禁用, 1=启用.", FCVAR_NOTIFY);
    g_hFile     = CreateConVar("sm_advertisements_file",     "advertisements.txt", "设置广告文件名称.", FCVAR_NOTIFY);
    g_hInterval = CreateConVar("sm_advertisements_interval", "30",                 "设置广告的播放间隔/秒.", FCVAR_NOTIFY);
    g_hSoundFile = CreateConVar("sm_advertisements_soundfile", "ui/beepclear.wav", "设置显示广告时播放的声音(相对于sound/目录, 留空=禁用).", FCVAR_NOTIFY);
