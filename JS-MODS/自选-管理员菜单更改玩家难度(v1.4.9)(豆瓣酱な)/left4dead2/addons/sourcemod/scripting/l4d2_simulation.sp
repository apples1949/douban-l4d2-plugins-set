 ...
 *	v1.4.9
 *
 *	1:获取的难度名称和错误信息改成使用用户输出的缓冲区长度(大佬的意思是不这样可能会有问题).
 *
 */
...
int SetNativeCustomizeDifficulty(Handle plugin, int numParams)
{
	...
	maxlength[0] = GetNativeCell(4);
	maxlength[1] = GetNativeCell(6);
	char[] sName = new char[maxlength[0]];
	char[] error = new char[maxlength[1]];
	...
	if(value >= -1 && value <= 3)
	{
		if(IsValidClient(client))
		{
			switch(GetClientTeam(client))
			{
				case 1:
				{
					if(!IsFakeClient(client))
					{
						...
						if(iBot != 0)
						{
							...
							SetNativeString(3, sName, maxlength[0]);
							SetNativeString(4, error, maxlength[1]);
				...
				case 3:
				{
					strcopy(error, MAX_LENGTH, "该功能只限生还者使用");
					SetNativeString(4, error, maxlength[1]);
					return 0;
				}
			}
		...
		{
			strcopy(error, MAX_LENGTH, "该功能只限真实玩家使用");
			SetNativeString(4, error, maxlength[1]);
		}
	}
	else
