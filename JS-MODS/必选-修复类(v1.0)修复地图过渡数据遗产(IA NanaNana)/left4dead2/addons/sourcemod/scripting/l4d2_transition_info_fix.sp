...
public void OnPluginStart()
{
	...
	if((z = DHookCreateFromConf(h, "ChangeLevelNow"))) 
		DHookEnableDetour(z, true, DH_ChangeLevelNow);
	else 
		SetFailState("Detour ChangeLevelNow invalid.");

	if(!(pCDirector = GameConfGetAddress(h, "CDirector"))) 
		SetFailState("Address CDirector invalid.");

	StartPrepSDKCall(SDKCall_Raw);
	PrepSDKCall_SetFromConf(h, SDKConf_Signature, "ServerShutdown");
	if(!(hServerShutdown = EndPrepSDKCall())) 
		SetFailState("Signature ServerShutdown invalid.");
