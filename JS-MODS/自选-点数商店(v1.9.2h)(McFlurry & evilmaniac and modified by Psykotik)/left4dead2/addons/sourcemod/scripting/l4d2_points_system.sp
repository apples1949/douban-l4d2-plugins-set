...
void InitItemCosts() {
	...
	g_eGeneral.cItemCosts[CostAWP] 				= CreateConVar("l4d2_points_awp", "800", "购买awp狙击枪需要多少积分");
	g_eGeneral.cItemCosts[CostScout] 			= CreateConVar("l4d2_points_scout", "500", "购买侦察狙击步枪(鸟狙)需要多少积分");
	...
	g_eGeneral.cItemCosts[CostGrenade] 			= CreateConVar("l4d2_points_grenade", "800", "购买榴弹发射器需要多少积分");
	g_eGeneral.cItemCosts[CostM60] 				= CreateConVar("l4d2_points_m60", "1100", "购买M60机枪需要多少积分");
	...
	g_eGeneral.cItemCosts[CostJoinSurvivors] 	= CreateConVar("l4d2_points_join_survivors", "150", "购买一次加入生还者需要多少积分");
}
...
void InitSpecialCosts() {
	...
	g_eSpecial.cfree_control 					= CreateConVar("l4d2_points_free_control", "800", "购买当前章节免疫控制需要多少积分");
	g_eSpecial.cplayer_teleport 				= CreateConVar("l4d2_points_player_teleport", "300", "购买传送到队友身边需要多少积分");
	g_eSpecial.cweapon_reload 					= CreateConVar("l4d2_points_weapon_reload", "800", "购买当前章节免换弹夹需要多少积分");
}
...
void BuildBuyMenu(int client) {
	switch (GetClientTeam(client)) {
		...
		case 3: {
			...
			if (g_eGeneral.cItemCosts[CostJoinSurvivors].IntValue > -1) {
				FormatEx(info, sizeof info, "%T", "Human", client);
				menu.AddItem("Human", info);
			}
			menu.Display(client, MENU_TIME_FOREVER);
		}
	}
