...
#define PLUGIN_VERSION	"1.4.4"	//插件的版本.
...
{	
	char sError[1024];
	if (!(g_dbSQL = SQLite_UseDatabase("l4d2_banned", sError, sizeof sError)))//创建指定名称的数据库.
		SetFailState("Could not connect to the database \"l4d2_banned\" at the following error:\n%s", sError);

	SQL_FastQuery(g_dbSQL, "CREATE TABLE IF NOT EXISTS l4d2_banned(SteamID NVARCHAR(32) NOT NULL DEFAULT '', banreason NVARCHAR(32) NOT NULL DEFAULT '', bantime INT NOT NULL DEFAULT 0, timestamp INT NOT NULL DEFAULT 0);");
}
//读取所有玩家数据.
...
void SQLQueryCallback(Database db, DBResultSet results, const char[] error, DataPack hPack) 
{
	...
	{	
		...
		if(iBanTime <= 0)
		{
			KickEntity(auth, "你已被封禁.\n封禁时长:永久封禁.\n封禁时间:%s.\n封禁原因:%s", GetTimeStampDate(iTimeStamp), sBanReason);
		}
		else
		{
			if(iBanTime * 60 + iTimeStamp > iNowTime)
				KickEntity(auth, "你已被封禁.\n剩余时长:%s.\n封禁时长:%s.\n封禁原因:%s", StandardizeTime((iBanTime * 60) - (iNowTime - iTimeStamp)), StandardizeTime(iBanTime * 60), sBanReason);
			else
			{
				if(write)
					SQL_Save(time, auth, reason[0] == '\0' ? "未填写封禁原因" : reason);//如果已存在数据就更新.
				else
				{
					...
					if(client != 0)
					{
						...
						strcopy(g_eBanned[client].sBanTime, sizeof(IsBanned::sBanTime), GetTimeStampDate(iTimeStamp));
	...
	else 
	{
		...
		{
			char query[1024];
			FormatEx(query, sizeof query, "INSERT INTO l4d2_banned(SteamID, banreason, bantime, timestamp) VALUES ('%s', '%s', %d, %d);", auth, reason[0] == '\0' ? "未填写封禁原因" : reason, time, GetTime());
			SQL_FastQuery(g_dbSQL, query);

			if(time <= 0)
				KickEntity(auth, "你已被封禁.\n封禁时长:永久封禁.\n封禁原因:%s", reason[0] == '\0' ? "未填写封禁原因" : reason);
			else
				KickEntity(auth, "你已被封禁.\n封禁时长:%s.\n封禁原因:%s", StandardizeTime(time * 60), reason[0] == '\0' ? "未填写封禁原因" : reason);
		}
	}
	delete hPack;
}
...
//更新玩家数据.
stock void SQL_Save(int time, char[] auth, char[] reason) 
{
	if (!g_dbSQL)
		return;

	char query[1024];
	FormatEx(query, sizeof query, "UPDATE l4d2_banned SET banreason = '%s', bantime = %d, timestamp = %d WHERE SteamID = '%s';", reason[0] == '\0' ? "无" : reason, time, GetTime(), auth);
	SQL_FastQuery(g_dbSQL, query);

	if(time == 0)
		KickEntity(auth, "你已被封禁.\n封禁时长:永久.\n封禁原因:%s", reason[0] == '\0' ? "无" : reason);
	else
		KickEntity(auth, "你已被封禁.\n封禁时长:%s.\n封禁原因:%s", StandardizeTime(time * 60), reason[0] == '\0' ? "无" : reason);
}
//格式化时间戳为日期格式.
stock char[] GetTimeStampDate(int iTimeStamp) 
{
	char sTimer[128];
	FormatTime(sTimer, sizeof(sTimer), "%Y-%m-%d %H:%M:%S", iTimeStamp);
	return sTimer;
}
