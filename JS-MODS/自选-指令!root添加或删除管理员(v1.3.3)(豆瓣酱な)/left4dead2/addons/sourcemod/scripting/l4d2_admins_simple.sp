...
{
	RegConsoleCmd("sm_root", Command_AddRemoveAdmin, "打开添加或删除管理员菜单.");
...
void IsListRealSurvivor(int client, int index)
{
	...
	for (int i = 0; i < iLists; i++)
	{
		...
		ImplodeStrings(sInfo, sizeof(sInfo), "|", sMerge, sizeof(sMerge));//打包字符串.
	...
	for(int i = 1; i <= MaxClients; i++)
	{
		if(IsClientInGame(i) && !IsFakeClient(i) && GetClientAuthId(i, AuthId_Steam2, SteamId, sizeof(SteamId)))
		{
			...
			ImplodeStrings(sData, sizeof(sData), "|", sMerge, sizeof(sMerge));//打包字符串.
			menu.AddItem(sMerge, sName);
		}
	}
...
{
	char line[128], sInfo[256], sData[4][128];
	ExplodeString(sItem, "|", sData, sizeof(sData), sizeof(sData[]));//拆分字符串.
	Menu menu = new Menu(Menu_HandlerFunction);
	FormatEx(line, sizeof(line), "确认%s[%s]的权限?", strcmp(sData[3], "添加", false) == 0 ? "添加" : "删除", sData[2]);
	strcopy(sData[0], sizeof(sData[]), sItem);
	IntToString(g_iSelection, sData[1], sizeof(sData[]));
	strcopy(sData[2], sizeof(sData[]), "内容");
	strcopy(sData[3], sizeof(sData[]), "内容");
	ImplodeStrings(sData, sizeof(sData), "‖", sInfo, sizeof(sInfo));//打包字符串.
	...
	switch(action)
	{
		...
		case MenuAction_Select:
		{
			...
			if(menu.GetItem(itemNum, sItem, sizeof(sItem)))
			{
				...
				{
					IsListRealSurvivor(client, StringToInt(sInfo[1]));
				}
			}
		}
	}
		...
		while(!fFile.EndOfFile())
		{
			...
			if(StrContains(szLine, sInfo[0]) == -1)//对比字符串.
			{
				...
				if(IsValidClient(target) && client != target)
					PrintHintText(target, "服管删除了你的管理员.");
			}
		}
		delete fFile;
		delete fTempFile;
...
void IsWriteLine(DataPack hPack)
{
	...
	int target = GetPlayersClient(sInfo[0]);
	if(IsValidClient(target) && client != target)
		PrintHintText(target, "服管添加你为管理员[%s].", sInfo[1]);
	delete fFile;
	delete hPack;
	IsListRealSurvivor(client, g_iSelection);
}
