 ...
 *	V2.4.10
 *
 *	1:随机密码删除了数字0,好像密码第一位是0的话会被忽略掉导致密码错误.
 *  2:特别说明,如果是默认密码则会在插件加载后重新随机新密码,如果是linux系统请注意文件权限问题.
 *
 *	V2.4.11
 *
 *	1:更改非指定玩家使用!root命令时的反馈提示使其更易懂.
 *
 */
...
#define PLUGIN_VERSION	"2.4.11"	//版本号.
...
void Command_AdminList(int client, int args)
{
	if(strcmp(g_sSteamId, DefaultSteamId, false) != 0)//对比字符串.
	{
		if(IsAcquirePlayerRights(GetSteamId(client)))
			...
			PrintToChat(client, "\x04[提示]\x05该命令只限指定的玩家使用(你的ID为:%s).", GetSteamId(client));
	}
	else
	{
		switch (args)
