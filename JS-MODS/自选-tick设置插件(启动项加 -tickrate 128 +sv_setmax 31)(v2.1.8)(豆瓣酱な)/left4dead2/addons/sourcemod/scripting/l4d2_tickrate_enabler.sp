...
void SetServerCvarValues(int iMaxTick, int iMaxFps, int iMinRatio, int iMaxRatio, int iSplitrate, float fFrequency, float fMaxcleartime)
{
	...
	if(fFrequency <= 0)//这里使用自动设置.
	{
		if(iMaxTick <= 60)
			...
			fFrequency = 0.024 - (0.00035 * (iMaxTick - 60));
		else if(iMaxTick >= 100)
			fFrequency = 0.01;
	}
	SetConVarFloat(FindConVar("nb_update_frequency"), fFrequency, false, false);//设置服务器世界的更新频率(默认值:0.1),数值越低丧尸和女巫的更新频率越高,非常耗费CPU. 0=自动设置.
