...
float g_fCoord[] = {0.00, 0.055, 0.110, 0.160, 0.215, 0.265, 0.315, 0.365};//设置每个HUD类型的预设坐标.

int    g_iDisplayNumber, g_iPlayerNum, g_iChapterTotal[2], g_iCumulativeTotal[2], g_iKillSpecial[MAXPLAYERS+1], g_iHeadSpecial[MAXPLAYERS+1], g_iKillZombie[MAXPLAYERS+1], g_iDmgHealth[2][MAXPLAYERS+1];

int    g_iSurvivorHealth, g_iMaxReviveCount, g_iPlayersNumber, g_iShowServerName, g_iShowServerTimer, g_iShowServerNumber, g_iShowServerTime, g_iFakeRanking, g_iReportTime, g_iTypeRanking, g_iDispRanking, g_iInfoRanking;
ConVar g_hSurvivorHealth, g_hMaxReviveCount, g_hPlayersNumber, g_hShowServerName, g_hShowServerTimer, g_hShowServerNumber, g_hShowServerTime, g_hFakeRanking, g_hTypeRanking, g_hDispRanking, g_hInfoRanking;
...
void IsKillLeaderboards()
{
	...
	char[][][] g_sData = new char[sizeof(g_sTitle)][assister_count + 1][maxStringLength];//创建动态大小的字符串.

	for (int z = 0; z < sizeof(g_sTitle); z++)
		strcopy(g_sData[z][0], maxStringLength, g_sTitle[z]);
	...
	for (int x = 0; x < assister_count; x++)
	{
		...
		if (IsValidClient(client))//因为要显示闲置玩家的数据,所以这里不要判断团队.
		{
			...
			strcopy(g_sData[0][j],	maxStringLength, GetSurvivorStatus(Player != 0 ? Player : client));

			IntToString(iHealth,	g_sData[1][j], maxStringLength);
			IntToString(iKill,		g_sData[2][j], maxStringLength);
			IntToString(iHead,		g_sData[3][j], maxStringLength);
			IntToString(iZombie,	g_sData[4][j], maxStringLength);
			IntToString(iHurt,		g_sData[5][j], maxStringLength);
			IntToString(iDmg,		g_sData[6][j], maxStringLength);
			strcopy(g_sData[7][j], maxStringLength, GetTrueName(Player != 0 ? Player : client));

			//IntToString(999,	g_sData[1][j], maxStringLength);
			//IntToString(9999,	g_sData[2][j], maxStringLength);
			//IntToString(9999,	g_sData[3][j], maxStringLength);
			//IntToString(9999,	g_sData[4][j], maxStringLength);
			//IntToString(9999,	g_sData[5][j], maxStringLength);
			//IntToString(9999,	g_sData[6][j], maxStringLength);
			//
			//strcopy(g_sData[7][j], maxStringLength, "这是:一个测试信息输出.");
	...
	for (int j = 1; j < ranking_count; j++)
		strcopy(g_sData[7][j], maxStringLength, GetDealName(g_sData[7][j], minlength));
	...
	for (int y = 1; y < ranking_count; y++)
	{
		for (int h = 0; h < quantity; h++)
		{
			...
			if(iMax[h] > 0)
				Format(g_sData[h + 1][y], maxStringLength, "%s%s", GetAddSpacesMax(iMax[h], " "),  g_sData[h + 1][y]);//这里不能使用FormatEx
		}
	}

	char g_sInfo[sizeof(g_sTitle)][256];
	for (int y = 0; y < sizeof(g_sTitle); y++)
		ImplodeStrings(g_sData[y], assister_count + 1, "\n", g_sInfo[y], sizeof(g_sInfo[]));//打包字符串.
	
	float g_fPos = 0.03;
	float value = 0.035 * (assister_count + 1) + 0.0035 * (assister_count + 1);//根据最大行数计算所需的宽度.
	...
	if(g_iTypeRanking & Number_1)
	{
		...
		HUDPlace(HUD_LEFT_BOT, g_fCoord[iHudCoord],g_fPos,1.0,value);
		iHudCoord += 1;
	}
	if((g_iTypeRanking & Number_2))
	{
		if(g_iDispRanking == 0 || !(g_iDispRanking & Number_1))
		{
			...
			HUDPlace(HUD_MID_BOT, g_fCoord[iHudCoord],g_fPos,1.0,value);
			iHudCoord += 1;
		}
		else
		{
	...
	if((g_iTypeRanking & Number_4))
	{
		if(g_iDispRanking == 0 || !(g_iDispRanking & Number_2))
		{
			...
			HUDPlace(HUD_RIGHT_TOP, g_fCoord[iHudCoord],g_fPos,1.0,value);
			iHudCoord += 1;
		}
		else
		{
	...
	if ((g_iTypeRanking & Number_8))
	{
		if(g_iDispRanking == 0 || !(g_iDispRanking & Number_4))
		{
			...
			HUDPlace(HUD_SCORE_3, g_fCoord[iHudCoord],g_fPos,1.0,value);
			iHudCoord += 1;
		}
		else
		{
	...
	if((g_iTypeRanking & Number_16))
	{
		if(g_iDispRanking == 0 || !(g_iDispRanking & Number_8))
		{
			...
			HUDPlace(HUD_TICKER, g_fCoord[iHudCoord],g_fPos,1.0,value);
			iHudCoord += 1;
		}
		else
		{
	...
	if((g_iTypeRanking & Number_32))
	{
		if(g_iDispRanking == 0 || !(g_iDispRanking & Number_16))
		{
			...
			HUDPlace(HUD_FAR_RIGHT, g_fCoord[iHudCoord],g_fPos,1.0,value);
			iHudCoord += 1;
		}
		else
		{
	...
	if((g_iTypeRanking & Number_64))
	{
		if(g_iDispRanking == 0 || !(g_iDispRanking & Number_32))
		{
			...
			HUDPlace(HUD_SCORE_2, g_fCoord[iHudCoord],g_fPos,1.0,value);
			iHudCoord += 1;
		}
		else
		{
	...
	if(g_iTypeRanking & Number_128)
	{
		if(ranking_count <= iSlotList)
		{
			...
			HUDPlace(HUD_RIGHT_BOT, g_fCoord[iHudCoord],g_fPos,1.0,value);
		}
		else//两个插槽拼接显示,但是有个奇怪的问题,必须15名生还者都显示的时候才能正常对齐,不然少一个生还者坐标就向下移动一点,很奇怪的问题(已解决:是因为字符串数组数量的问题,改成动态数组后成功解决了,更正:是因为字符数量超过上限导致的).
		{
			char[][] sData = new char[ranking_count][maxStringLength];//创建动态大小的字符串.
			ExplodeString(g_sInfo[7], "\n", sData, ranking_count, maxStringLength);//拆分字符串.
			...
			for (int i = 0; i < ranking_count; i++)
			{
				if(i <= iSlotList)
				{
					strcopy(sInfo1[i], maxStringLength, sData[i]);
					strcopy(sInfo2[i], maxStringLength, " ");
				}
				else
				{
					strcopy(sInfo1[i], maxStringLength, " ");
					strcopy(sInfo2[i], maxStringLength, sData[i]);
				}
			}
			char sName[2][256];
			...
			HUDPlace(HUD_SCORE_TITLE, g_fCoord[iHudCoord],g_fPos,1.0,value);
...
void IsShowServersNumber()
{
	...
	if(g_iShowServerNumber & Number_1)
	{
		...
		if(iNumber[0] > 0)
			FormatEx(sNumber[0], sizeof(sNumber[]), "➣人数:%d/%d\n", iNumber[0], iNumber[0] > iMaxPlayers ? iNumber[0] : iMaxPlayers);
	}
