 ...
 *	v1.1.5
 *
 *	1:自定义女巫编号插件删除了获取爆头秒杀击杀API功能,这里不做限制了,爆头和秒杀可以同时触发.
 *
 */
#pragma semicolon 1
//強制1.7以後的新語法
...
public void Event_Witchkilled(Event event, const char[] name, bool dontBroadcast)
{
	...
	if(IsValidClient(client) && GetClientTeam(client) == 2)
	{
		...
		int iReward = g_iOneshotWitch;
		if(oneshot != 0)
		{
			if(SetSurvivorHealth(client, GetRewardHealth(iReward), g_iLimitHealth))
			{
				if(iReward > 0)
					PrintToChatAll("\x04[提示]\x03%s\x05秒杀了\x03%s\x04,\x05奖励\x03%d\x05点血量.", GetTrueName(client), GetWitchName(iWitchid), GetRewardHealth(iReward));
			}
			else
			{
				if(iReward > 0)
					PrintToChatAll("\x04[提示]\x03%s\x05秒杀了\x03%s\x04,\x05血量已达\x03%d\x05上限.", GetTrueName(client), GetWitchName(iWitchid), g_iLimitHealth);//聊天窗提示.
			}
		}
	}
}
						...
						if(iReward > 0)
							if(iBot != 0)
							{
								if(IsValidClient(iBot))
									PrintToChat(iBot, "\x04[提示]\x05%s了\x03%s\x04,\x05奖励\x03%d\x05点血量.", sType, GetWitchName(entityid), GetRewardHealth(iReward));
							}
							else
								PrintToChat(attacker, "\x04[提示]\x05%s了\x03%s\x04,\x05奖励\x03%d\x05点血量.", sType, GetWitchName(entityid), GetRewardHealth(iReward));
					}
					else
					{
						if(iReward > 0)
							...
							{
								if(IsValidClient(iBot))
									PrintToChat(iBot, "\x04[提示]\x05%s了\x03%s\x04,\x05血量已达\x03%d\x05上限.", sType, GetWitchName(entityid), g_iLimitHealth);//聊天窗提示.
							}
							else
								PrintToChat(attacker, "\x04[提示]\x05%s了\x03%s\x04,\x05血量已达\x03%d\x05上限.", sType, GetWitchName(entityid), g_iLimitHealth);//聊天窗提示.
					}	
				}
				else
				{
					if(iReward > 0)
						...
						{
							if(IsValidClient(iBot))
								PrintToChat(iBot, "\x04[提示]\x05%s了\x03%s\x04.", sType, GetWitchName(entityid));//聊天窗提示.
						}
						else
							PrintToChat(attacker, "\x04[提示]\x05%s了\x03%s\x04.", sType, GetWitchName(entityid));//聊天窗提示.
