	...
	if(IsValidClient(attacker) && GetClientTeam(attacker) == 2)
	{
		if(IsValidClient(client) && GetClientTeam(client) == 3 && IsPlayerAlive(client) && GetEntProp(client, Prop_Send, "m_zombieClass") == 8)
		{
			if (IsPlayerState(client))//判断坦克是正常状态.
			{
				...
				g_iSurvivorTankHurt[!iBot ? attacker : iBot][client] += iDmg;
...
void IsTankThink(int client)
{
	if (IsClientInGame(client) && GetClientTeam(client) == 3 && IsPlayerAlive(client) && GetEntProp(client, Prop_Send, "m_zombieClass") == 8 && IsClientInKickQueue(client))
	{
		...
		if(IsFakeClient(client))
		{
			...
			FormatEx(sName, sizeof(sName), "\x03%N", client);
		if (g_iTankRanking > 0)
			IsTankDamageSort(client, sName);
...
public void Event_PlayerDeath(Event event, const char[] name, bool dontBroadcast)
{
	...
	if(IsValidClient(client) && GetClientTeam(client) == 3 && GetEntProp(client, Prop_Send, "m_zombieClass") == 8)
	{
		...
		{
			if(client != attacker)//坦克自杀的情况下攻击者是坦克自己.
				g_iSurvivorTankHurt[attacker][client] += g_iTankHurt[client];//坦克死亡后把剩余血量+给击杀者.
...
//坦克伤害排行榜.
void IsTankDamageSort(int client, char[] sName)
{
	int assister_count;
	...
	if (assister_count > 0)
	{
		...
		PrintToChatAll("\x04坦克%s\x05总血量\x04:\x03%d\x05HP\x04.\n\x05显示伤害排名\x04:\x03(\x05总伤害\x04:\x05%d\x03)", sName, g_iTankHP[client], GetTotalDamage(client));
		for (int x = 0; x < g_iTankRanking; x++)
		{
			...
			if (IsValidClient(attacker))
			{
				...
				FormatEx(sData[x][1], MAX_SIZE, "%.1f", GetDamagePercentage(damage, client));
		...
		{ 
			...
			GetAddSpaces(iMax[0], " "), sData[x][0], GetAddSpaces(iMax[0], " "), GetAddSpaces(iMax[1], " "), sData[x][1], 
			GetAddSpaces(iMax[1], " "), GetAddSpaces(iMax[2], " "), sData[x][2], GetAddSpaces(iMax[2], " "), sData[x][3]);
		}
	}
}
...
//填入对应数量的空格.
char[] GetAddSpaces(int Value, char[] sContent)
{
	char sBlank[256];

	if(Value > 0)
	{
		char[][] sData = new char[Value][MAX_SIZE];//动态数组.
		for (int i = 0; i < Value; i++)
			strcopy(sData[i], MAX_SIZE, sContent);
		ImplodeStrings(sData, Value, "", sBlank, sizeof(sBlank));//打包字符串.
	}
	return sBlank;
}

//百分比取整.
float GetDamagePercentage(int iDamage, int client)
{
	return float(iDamage)/float(g_iTankHP[client]) * 100.0;
}
